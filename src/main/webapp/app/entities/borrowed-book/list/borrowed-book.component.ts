...
import { BorrowedBookService, EntityArrayResponseType } from '../service/borrowed-book.service';
...
export class BorrowedBookComponent implements OnInit {
  ...
  trackId = (item: IBorrowedBook): number => this.borrowedBookService.getBorrowedBookIdentifier(item);
      ...
      .pipe(
        ...
        tap(() => {
          if (this.borrowedBooks().length === 0) {
            ...
            this.borrowedBooks.set(this.refineData(this.borrowedBooks()));
          }
        }),
      )
      .subscribe();
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.borrowedBooks.set(this.refineData(dataFromBody));
  }

  protected refineData(data: IBorrowedBook[]): IBorrowedBook[] {
