...
import { sampleWithRequiredData } from '../book-author.test-samples';
...
describe('BookAuthor Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.bookAuthors()[0]).toEqual(expect.objectContaining({ id: 16660 }));
  });

  describe('trackId', () => {
    it('Should forward to bookAuthorService', () => {
      const entity = { id: 16660 };
      jest.spyOn(service, 'getBookAuthorIdentifier');
      const id = comp.trackId(entity);
      expect(service.getBookAuthorIdentifier).toHaveBeenCalledWith(entity);
